...
const PhotoPage = () => {
    ...
    const {query:{photo}}=router
    const [photoItem,setPhotoItem]=React.useState()
    const {title,setSelect,setTitle,keyword,imageList,setimageList} = React.useContext(FunctionalsContent)
   console.log(photo);
   React.useEffect(()=>{
    if(photo){
            fetchImage(`https://api.unsplash.com/photos/${photo}?`)
            .then(res=>{
                setPhotoItem(res)
                console.log(res);
        ...
        <Layout>
            <PhotoContainer bg={color}>
                ...
                <div className="container pt-5 pb-5">
                    <div className="row ">
                        <div className="col-lg-6"><img src={urls.regular} className="img-fluid"/></div>
